...
class main():
            ...
            for line in fileinput.input(self.processFile, inplace=1):
                if len(re.findall(regex, line)) > 0:
                    if "bundle" in line or "ip" in line:
                        line = line.replace(line, '"' + re.findall(regex, line)[0] + '"' + ":" + '"' + str(
                            testData.get(values.name)) + '",')
    ...
    def teardownDbData(self):
        with open(self.cleanData) as deletes:
            for lines in deletes:
                ...
                connectToPostgres(os.environ['POSTGRES_HOST'], os.environ['POSTGRES_USER'], os.environ['POSTGRES_PW'],
                                  os.environ['POSTGRES_PORT'], lines)
    ...
    def insertDataIntoTables(self):
        ...
        for inserts in stmts:
            if len(inserts) > 0:
                print(inserts)
                if "creative" in inserts and len(jsonData.get("creativeMetadata")) > 0:
                    ins1 = inserts.split("VALUES")[0]
                    crid = inserts.           split("VALUES")[1].split(",")[2]
                    adid = inserts.split("VALUES")[1].split(",")[3]
                    height = jsonData.get("creativeMetadata")[0].get("height")
                    width = jsonData.get("creativeMetadata")[0].get("width")
                    duration = jsonData.get("creativeMetadata")[0].get("mediaDurationSeconds")
                    insert2 = "(" + str(width) + " , " + str(height) + " , " + str(crid) + " , " + str(
                        adid) + " , " + str(duration) + " )"
                    inserts = ins1 + " VALUES " + insert2
                connectToPostgres(os.environ['POSTGRES_HOST'], os.environ['POSTGRES_USER'], os.environ['POSTGRES_PW'],
                                  os.environ['POSTGRES_PORT'], inserts)
        return stmts
...
#     main("MagniteBidRequestIpValidation").insertDataIntoTables()
